...
Shader "KYU3D/Scene/WaterTransparent"
{
	Properties
	{
		...
		_LightmapWave( "_LightmapWave" , float ) = 0.003
		[HideInInspector]_LightmapVaryB( "_LightmapVaryB", float ) = 0.1
		[HideInInspector]_LightmapVaryK( "_LightmapVaryK", float ) = 0.5
	}
	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag( v2f i ) : COLOR
			{
				...
				half3 wc = lerp( _Color2, _Color1, saturate( max( nv, 0 ) * _ColorVaryK + _ColorVaryB ) );
				half s = pow( max( dot( normalize( v + i.light ), n ), 0 ), _Shininess ) * _SpecularIntensity;

				half4 c;
				c.rgb = ( max( ( r.rgb - wc ) * f, 0 ) + wc + s * _Specular );
